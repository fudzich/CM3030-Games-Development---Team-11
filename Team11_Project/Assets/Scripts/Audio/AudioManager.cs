...
public class AudioManager : MonoBehaviour
{
    ...
    public void PlayContSoundEffect(AudioType type)
    {
        if (!_audioDictionary.TryGetValue(type, out Audio sound))
        {
            Debug.LogWarning($"Continuous sound effect {type} not found!");
            return;
        }


        var container = new GameObject($"Audio_{type}");
        _musicSource = container.AddComponent<AudioSource>();
        _musicSource.loop = true;
    ...
    public void StopMusic(GameObject audioOject)
    {
        Destroy(audioOject);
    }
}
